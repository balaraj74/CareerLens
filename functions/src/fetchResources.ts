 ...
 */
async function scrapeNPTEL(searchTerm: string): Promise<ScrapedCourse[]> {
  try {
    ...
    $('.course-card').slice(0, 10).each((_i: number, el: any) => {
 ...
 */
async function scrapeCoursera(searchTerm: string): Promise<ScrapedCourse[]> {
  try {
    ...
    $('li.cds-9').slice(0, 10).each((_i: number, el: any) => {
 ...
 * Scrape AWS Training
 */
async function scrapeAWS(): Promise<ScrapedCourse[]> {
  try {
    ...
    $('div.lb-content-item').slice(0, 10).each((_i: number, el: any) => {
 ...
 * Scrape GCP Training
 */
async function scrapeGCP(): Promise<ScrapedCourse[]> {
  try {
    ...
    $('div.cloud-card').slice(0, 10).each((_i: number, el: any) => {
      const title = $(el).find('h3').text().trim();
      const href = $(el).find('a').attr('href');
      const description = $(el).find('p').text().trim();
 ...
 */
async function storeCourses(courses: ScrapedCourse[]): Promise<number> {
  let stored = 0;
 ...
 */
export const fetchResourcesScheduled = functions.pubsub
  .schedule('0 */12 * * *')
 ...
 */
export const fetchResourcesManual = functions.https.onRequest(async (req, res) => {
  try {
    console.log('Manual resource scraping triggered...');

    const searchTerm = req.query.search as string || 'programming';
